...
export default class ListingComment extends Component {
    constructor(props) {
        super(props)
    }
    ...
    componentDidMount() {
        axios.get('http://localhost:5000/api/listingcomment')
        .then ((response) =>{
            this.setState({
    ...
    render() {
        return (
            <>
            {this.state.comments.filter(comment => comment.listingId === this.props.listingId).map( comment => (
